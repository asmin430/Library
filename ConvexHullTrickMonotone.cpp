...
struct ConvexHullTrickMonotone{
    static_assert(is_same_v<T, long long> || is_floating_point_v<T>);
    struct Line{
        T a, b;
        int idx;
    };
    deque<Line> H;
    int nxt_idx = 0;
    ConvexHullTrickMonotone() = default;
    ...
    inline bool check(const Line &a, const Line &b, const Line &c){
        ...
        return D(b.a - a.a) * sgn(c.b - b.b) / D(abs(b.b - a.b)) >= D(c.a - b.a) * sgn(b.b - a.b) / D(abs(c.b - b.b));
    }
    void add(T a, T b, int idx = -1){
        ...
        if(H.front().a <= a){
            ...
            while(H.size() >= 2 && check(H[H.size() - 2], H.back(), L)){
                H.pop_back();
            }
            H.emplace_back(L);
        }
    }
    inline T get_y(const Line &a, const T &x){
        return a.a * x + a.b;
    }
